              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <SectionCard className="bg-white">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-[11px] font-semibold text-tecdia-text/60 uppercase tracking-widest">Category</label>
                      <div className="relative">
                        <select
                          value={form.category}
                          onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                          className={`w-full appearance-none bg-tecdia-background border border-tecdia-border rounded-xl py-3 pl-4 pr-10 text-sm focus:outline-none focus:border-tecdia-accent focus:ring-1 focus:ring-tecdia-accent/20 transition-all ${form.category ? 'text-tecdia-text' : 'text-tecdia-text/40'}`}
                        >
                          <option value="">Select a category…</option>
                          {CATEGORY_OPTIONS.map(c => (
                            <option key={c} value={c} className="text-tecdia-text">{c}</option>
                          ))}
                        </select>
                        <ChevronDown size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-tecdia-text/40" />
                      </div>
                    </div>
