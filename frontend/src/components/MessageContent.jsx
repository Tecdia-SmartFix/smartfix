...
const MessageContent = ({ content, isAI }) => {
  return (
    <div className={`prose max-w-none text-[15px] leading-relaxed ${isAI ? 'text-gray-900' : 'text-black'}`}>
      <ReactMarkdown
        ...
        components={{
            ...
            return inline ? (
              ...
              <div className="relative group my-4">
                <div className="flex items-center justify-between px-4 py-2 bg-tecdia-background border-b border-tecdia-border rounded-t-xl">
                  <span className="text-[11px] uppercase font-bold text-gray-600 tracking-widest">
                    {className?.replace('language-', '') || 'Code'}
                  </span>
                  <CopyButton text={codeText} />
                </div>
                <pre className="bg-tecdia-surface p-4 rounded-b-xl border border-t-0 border-tecdia-border overflow-x-auto font-mono text-sm leading-relaxed m-0 text-black" {...props}>
                  <code>{children}</code>
                </pre>
              </div>
            );
          ...
          h1: ({ children }) => <h1 className="text-xl font-bold mb-4 text-black">{children}</h1>,
          h2: ({ children }) => <h2 className="text-lg font-bold mb-3 text-black">{children}</h2>,
          h3: ({ children }) => <h3 className="text-md font-bold mb-2 text-black">{children}</h3>,
          strong: ({ children }) => <strong className="text-black font-semibold">{children}</strong>,
          blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-tecdia-accent/40 pl-4 py-1 italic text-gray-700 mb-4 bg-tecdia-accent/5 rounded-r-lg">
              {children}
            </blockquote>
          ),
        }}
